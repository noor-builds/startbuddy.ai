...
class _RoadmapViewState extends State<RoadmapView> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadTasks() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load roadmap tasks: $e'), backgroundColor: AppTheme.error),
      );
    }
  }
  ...
  Future<void> _generateAIRoadmap() async {
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300 && body['ok'] == true) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('AI Roadmap for ${stage.toUpperCase()} generated!')),
          );
        }
      } else {
        throw Exception(body['error']?['message'] ?? 'Failed to generate roadmap');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Roadmap error: $e'), backgroundColor: AppTheme.error),
        );
      }
    } finally {
      if (mounted) {
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  Text('Priority', style: GoogleFonts.roboto(fontSize: 13, color: AppTheme.textSecondaryDark)),
          ...
          child: Row(
            ...
            children: [
              Text(
                ...
                style: GoogleFonts.roboto(fontWeight: FontWeight.w500, color: AppTheme.textSecondaryDark),
              ),
              Row(
                children: [
                  IconButton(
                    icon: const Icon(Icons.add_circle_outline_rounded, color: AppTheme.accent, size: 24),
                  ...
                  ElevatedButton.icon(
                    style: ElevatedButton.styleFrom(
                      ...
                      textStyle: GoogleFonts.roboto(fontSize: 13, fontWeight: FontWeight.bold),
              ...
              : stageTasks.isEmpty
                  ? Center(
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(Icons.checklist_rounded, size: 48, color: Colors.white.withOpacity(0.12)),
                          ...
                          Text(
                            ...
                            style: GoogleFonts.roboto(fontSize: 12, color: AppTheme.textSecondaryDark.withOpacity(0.7)),
                          ),
                        ],
                      ),
                    )
